...
from inventario.modelos.proveedores import Proveedores
from inventario.modelos.movimiento_inventario import MovimientoInventario
...
class ExceptionsInventary:
    ...
    def listarProductos(self):
         ...
         for producto in self.listaProductos:
              print(f"{i}- {producto.nombreProducto}")
              i += 1
    ...
    def eliminarProveedor(self):
        self.mostrarProveedores()
        index = int(input("Ingresa el indice del proveedor que deseas eliminar: ")) - 1

        self.listProveedores.pop(index)
    ...
    def actualizarPrecio(self):
         ...
         self.listaProductos[index].precioProducto = nuevo
         ...
         if not self.listaProductos:
             ...
             for producto in self.listaProductos:
                 stock_data.append({
                     ...
                     "Proveedor": producto.proveedorProducto.getNombreProveedor(),
             ...
             #print("--- STOCK ACTUAL ---")
             #for producto in self.listaProductos:
                #print(f"Nombre del producto: {producto.nombreProducto}")
                #print(f"Stock actual: {producto.stockProducto}")
                #print(f"Proveedor del producto: {producto.proveedorProducto.getNombreProveedor()}")
                #print(f"Precio del producto: {producto.precioProducto}")
    ...
    def registrarEntradaSalida(self):
        ...
        match opcion:
            case 1:
                ...
                if self.listaProductos[index].stockProducto < cantidad:
                    print("Error: no hay suficiente stock")
                    return
                
                newStock = self.listaProductos[index].stockProducto - cantidad
                self.listaProductos[index].stockProducto = newStock
                fecha = datetime.now()

                nombre = self.listaProductos[index].nombreProducto
    ...
    def verHistorial(self):
        if not self.listaMovimientos:
            ...
            for mov in self.listaMovimientos:
                ...
                movs_data.append({
                    "Producto": mov.getNombreProducto(),
                    "Tipo de movimiento": mov.getMovimiento(),
                    "Cantidad": mov.getCantidad(),
                    "Fecha": fecha.strftime('%d/%m/%Y'),
                    "Hora": fecha.strftime('%H:%M')
                })

            df = pd.DataFrame(movs_data)
            df.index = df.index + 1
